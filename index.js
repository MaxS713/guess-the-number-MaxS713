...
try to guess it."
  ...
  let answer =
    await ask(`Pick a number between 1 and ${maxNumber} (inclusive), \
...
async function game() {
  ...
  if (numberOfGuesses === 0) {
    ...
    //Because we take the "floor" during calculations, the + 1 is needed so the guessing can be inclusive
    numberToGuess = Math.floor(maxNumber / 2); //the computer guesses the number right in the middle of the range
    numberOfGuesses++; //this keeps track of number of guesses so far
    console.log(`\nOk... Let me think...`);
...
and you (human) try to guess it!"
  ...
  let answer =
    await ask(`I will now pick a number between 1 and ${maxNumber} (inclusive), \
...
async function reverseGame() {
  ...
  if (numberOfGuesses === 0) {
    ...
    //I found out that you need to "parseInt" the value given -
    //else it sees it as a text string... I do it for every input that follows
  }
...
It took you ${numberOfGuesses} tries!`);
    ...
    numberOfGuesses--;
    guess = await ask(
      `Hmm.. this is outside the range of possibilities. Guess again! `
    );
    numberOfGuesses++;
    guess = parseInt(guess);
    reverseGame();
